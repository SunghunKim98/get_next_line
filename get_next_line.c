...
/*   Updated: 2021/05/17 22:21:45 by sungkim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*extract_remainder(char *str)
{
	...
	ft_strlcpy(p, str + size + 1, total_size - size + 2);
	free(str);
	return (p);
}
...
int		get_next_line(int fd, char **line)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || !line)
		return (-1);
	result_read = 1;
	while ((result_check = check(remainder)) == 0 && result_read > 0)
	{
